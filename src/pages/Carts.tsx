...
const Carts = () => {
  ...
  useEffect(() => {
    if(data){
        const filterProductQuantityByIds = Object.keys(productQuantity).filter(id => productQuantity[Number(id)] > 0).map(Number)
  ...
  return (
    <div className="mt-4 flex flex-col items-center px-5">
      {loading && <Loader />}
      {!loading && !error && carts?.length !== 0 && <CategoriesTitle title="Carts" />}
